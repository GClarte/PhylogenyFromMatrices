...
tr =rcoal(11)[[1]]
tr2=rcoal(4)[[1]]
tr$edge.length=300/hauteur(tr,12)*tr$edge.length
tr2$edge.length=200/hauteur(tr2,5)*tr2$edge.length
...
Xreel2 = list(
  transf(tr2, Trpossb[[1]], 0.01, rep(1 / 12, 12)),
  transf(tr2, Trpossb[[2]], 0.01, rep(1 / 12, 12)),
  transf(tr2, Trpossb[[3]], 0.001, rep(1 / 2, 2)),
  transf(tr2, Trpossb[[4]], 0.001, rep(1 / 2, 2))
)
...
Tpsreel2 = lapply(Xreel2, function(x) {
  lapply(x, function(z) {
    sort(runif(length(z), 0, 1))
  })
})
...
Lreelnb = matrix(rpois(100 * 20, .001 * tr$edge.length),
                 ncol = 20,
                 byrow = T)
Lreel = Lreelnb
...
Lreelnb2 = matrix(rpois(100 * 6, .001 * tr2$edge.length),
                 ncol = 6,
                 byrow = T)
Lreel2 = Lreelnb2
Lreel2[Lreelnb2 != 0] = rbeta(length(which(Lreelnb2 != 0)), Lreel2[Lreelnb2 !=
                                                                 0], 1)

D1 = evolarbre(tr, Xreel[[1]], rep(1 / 5, 5), 100, 12, 21, Trpossb[[1]], 5, .001, Lreel)
Dred1 = D1[, 1:11]
D2 = evolarbre(tr, Xreel[[2]], rep(1 / 5, 5), 100, 12, 21, Trpossb[[2]], 5, .001, Lreel)
Dred2 = D2[, 1:11]
D3 = evolarbre(tr, Xreel[[3]], rep(1 / 2, 2), 100, 12, 21, Trpossb[[3]], 2, .001, Lreel)
Dred3 = D3[, 1:11]
D4 = evolarbre(tr, Xreel[[4]], rep(1 / 2, 2), 100, 12, 21, Trpossb[[4]], 2, .001, Lreel)
Dred4 = D4[, 1:11]

D1b = evolarbre(tr2, Xreel2[[1]], rep(1 / 5, 5), 100, 5, 7, Trpossb[[1]], 5, .001, Lreel2)
Dred1b = D1b[, 1:4]
D2b = evolarbre(tr2, Xreel2[[2]], rep(1 / 5, 5), 100, 5, 7, Trpossb[[2]], 5, .001, Lreel2)
Dred2b = D2b[, 1:4]
D3b = evolarbre(tr2, Xreel2[[3]], rep(1 / 2, 2), 100, 5, 7, Trpossb[[3]], 2, .001, Lreel2)
Dred3b = D3b[, 1:4]
D4b = evolarbre(tr2, Xreel2[[4]], rep(1 / 2, 2), 100, 5, 7, Trpossb[[4]], 2, .001, Lreel2)
Dred4b = D4b[, 1:4]
Dtot = list(cbind(Dred1,Dred1b), cbind(Dred2,Dred2b), cbind(Dred3,Dred3b), cbind(Dred4,Dred4b))
...
Param=list(Trpossliste=passages,
           ...
           agemax=c(10,1/30),
           tiplabel=as.character(1:15),
           batches=c(lapply(1:(ncol(Dat[[1]])),function(x){nrow(Dat[[1]])})),
           ReconstructClade=list(),
           Cladeage=list(list(c(1,4),c(40,60)),list(c(13,15),c(20,60)))
)

VV2=SMCbruit(Dat,Param,Prior,30)

save.image(paste("SMCforetsimu2arbres", date(), ".RData"), version = 2)
