...
Data=read.csv("Tai_final.csv",stringsAsFactors = T,header = F)
...
quelleslangues = unique(Data[,1]) # TOFILL
...
quelmeanings = unique(Data[,2]) # TOFILL
...
for (i in 1:nch) {
  ...
  for (j in 1:length(quelleslangues)) {
    for (l in 1:length(quelmeanings)) {
      Dtot[[i]][l, j] = as.integer(Data[Data[,1] == quelleslangues[j]
                                     & Data[,2] == quelmeanings[l], qui[i]])
    }
  }
}
...
etats = c(length(levels(Data[,3])),length(levels(Data[,4])),length(levels(Data[,5])),length(levels(Data[,6]))) # TOFILL
...
Param = list(
  ...
  #or you can learn it, in which case you must put two aruyments that will be used as parameter of a gamma proposal
...
save.image(paste("Tai", date(), ".RData"), version = 2)
