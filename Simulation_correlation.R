...
D2=D1
for (j in 1:5){
  kk=which(tr$edge[,2]==j)
  for (i in 1:nrow(D1)){
    if (runif(1)<0.001*tr$edge.length[kk]){
      D2[i,j]=sample(1:5,1,prob=rep(1 / 5, 5))
    }
  }
}
Dred1 = D1[, 1:15]
...
Prior=list(Prirho=c(1,1000),
           ...
           pribruit=pribruit,#prior sur le bruit avec tout défini
             tipprior=list(c(8,12,13,15),c(9,11,10)))
#tipprior=list())
Param=list(Trpossliste=passages,
           ...
           ReconstructClade=list(c(10,9),c(6,2)),
             Cladeage=list(list( c(10,9),c(20,100)),list( c(1,2),c(20,100)),list( c(8,15),c(50,200)))
)

VV2=SMCbruit(Dat,Param,Prior,30)

save.image(paste("SMCforetsimu3corr", date(), ".RData"), version = 2)
